...
void main() {
	...
	if (VAR == 0) {
		...
		sdf = (1.0 - stroke(sdf, 0.0, borderSize, sdfCircle)) * 4.0;
	} else if (VAR == 1) {
		/* sdf circle with fill param adjusted by sdf circle */
		sdf = sdCircle(st, vec2(0.5));
		sdf = (1.0 - fill(sdf, 0.6, sdfCircle)) * 1.2;
	} else if (VAR == 2) {
		/* sdf circle with stroke param adjusted by sdf circle */
		sdf = sdCircle(st, vec2(0.5));
		sdf = stroke(sdf, 0.58, 0.015, sdfCircle) * 2.0;
	} else if (VAR == 3) {
		/* sdf circle with fill param adjusted by sdf circle */
	...
	vec3 color = mix(color1, color2, sdf);
